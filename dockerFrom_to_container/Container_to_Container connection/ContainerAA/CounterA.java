...
public class CounterA {
    ...
    public static void main(String[] args) throws Exception {
        ...
        server.createContext("/count-from-b", CounterA::handleIncrement);
        server.createContext("/value", CounterA::handleValue);

        server.start();
        System.out.println("Container A running on port 8080");
    }

    private static void handleIncrement(HttpExchange exchange) throws IOException {
        int value = counter.incrementAndGet();
        respond(exchange, "Counter A incremented to: " + value);
    }

    private static void handleValue(HttpExchange exchange) throws IOException {
        respond(exchange, "Counter A value: " + counter.get());
    }
